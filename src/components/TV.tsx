    ...
    <div className="container flex justify-center items-center mt-16">
      <div className={`tv ${powerOn ? "on" : ""}`}>
        <div className="television-container flex flex-col items-center z-[1]">
              ...
              <div className="television-screen-container border border-[#222] shadow-md rounded-[35px/25px] m-[10px] overflow-hidden flex justify-center items-center bg-[linear-gradient(70deg,#555_15%,transparent_30%),repeating-conic-gradient(#222_0_30deg,#333_60deg,#222_90deg)]">
                <div className="television-crt w-[99%] h-[95%] bg-[#111] shadow-md rounded-[85px/100px] overflow-hidden flex justify-center items-center">
                  <div className="television-screen bg-[#302d30] w-[95%] h-[95%] rounded-[30%] shadow-inner overflow-hidden relative">
                    ...
                    {/* TV Screen Content */}
                    <div className={`logo-container absolute z-[0] w-full h-full flex justify-center items-center`}>
                    ...
                    {/* TV Effects */}
                    <div className="noise w-full h-full bg-[linear-gradient(to_bottom,transparent,#aaa4,#8881,#6664,#4445,#2228,#4443,transparent),repeating-linear-gradient(transparent_0_2px,rgba(37,36,41,0.3)_2px_4px)] animate-scanlines absolute inset-0 z-[4]"></div>
                    
                    {/* Power Off Overlay */}
                    <div className={`off w-full h-full bg-[radial-gradient(#222,#1a1a1a,#111)] absolute top-0 transition-opacity duration-500 z-[5] ${powerOn ? "opacity-0" : "opacity-100"}`}></div>
                  </div>
                </div>
              </div>
              ...
              <div className="television-lateral grid grid-rows-[1.1fr_1fr] gap-[12px] my-[15px] ml-0">
                <div className="dial-container border-2 border-[#333] border-l-[#444] border-b-[#444] rounded-[5px] shadow-md flex flex-col justify-center gap-[6px] items-center">
                  ...
                  >
                    <div className="data-container flex justify-center w-full h-full">
                      {Array(12).fill(0).map((_, i) => (
                        <div key={`channel-${i}`} className="data text-[#888] text-[6px] shadow-sm absolute top-0 h-full" style={{ transform: `rotate(${i * 30}deg)`, transformOrigin: '50% 50%' }}>
                          #
                        </div>
                      ))}
                    </div>
                  ...
                  >
                    <div className="data-container flex justify-center w-full h-full">
                      {Array(12).fill(0).map((_, i) => (
                        <div key={`volume-${i}`} className="data text-[#444] text-[6px] shadow-sm absolute top-0 h-full" style={{ transform: `rotate(${i * 30}deg)`, transformOrigin: '50% 50%' }}>
                          #
                        </div>
                      ))}
                    </div>
                ...
                <div className="speaker-container grid grid-cols-6 gap-[1px_0] mb-[12px] mx-[2px]">
                  {Array(48).fill(0).map((_, i) => (
                    <div key={i} className="w-[10px] h-[10px] bg-[radial-gradient(#000,#222)] rounded-full border-b border-white"></div>
                  ))}
                </div>
              </div>
          ...
          {/* TV Base */}
          <div className="television-base w-[350px] h-[25px] bg-gradient-to-b from-[#b71a11] to-[#88110b] flex justify-between items-center shadow-md relative">
